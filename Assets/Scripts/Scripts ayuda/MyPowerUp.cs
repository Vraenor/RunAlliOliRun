...
public class MyPowerUp : MonoBehaviour {
	...
	void Start()
	{
		...
		powerUpLooping = GameObject.FindGameObjectWithTag ("PowerUpLoop").GetComponent<Loop_PowerUp> ();
	...
	void Update () 
	{
		if (Input.GetKeyDown(KeyCode.J)) 
		{
			...
			power_up = (GameObject) Instantiate(powerups[2],player.transform.position +  new Vector3(0.5f ,0.7f, player.transform.forward.z),player.transform.rotation); //Quaternion.identity);
		...
		if (Input.GetKeyDown(KeyCode.D) && n_of_powerup != -1 && posible_lanzar) {
			...
			case 4:	//Rocket straight
				power_up = (GameObject) Instantiate(powerups[2],player.transform.position +  new Vector3(0.5f ,0.45f, player.transform.forward.z),player.transform.rotation);
				power_up.SetActive(true);
				//Debug.Break();
				toggleOffPowerUpEffectAndImage();
				break;
		...
		if (player.GetComponent<ExamplePlayerController>().enabled == false) {
