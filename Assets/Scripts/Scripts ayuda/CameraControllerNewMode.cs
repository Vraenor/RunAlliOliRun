...
/// </summary>
public class CameraControllerNewMode : MonoBehaviour 
{
	public Transform target;											// the object we are following
	public Vector3 followOffset = new Vector3(0.0f, 2.0f, -3.0f);		// the offset away from the target that we attempt to keep
	...
	void FixedUpdate() 
	{
		...
		if (transform.position.y < -1.01f) {
			//Vector3 camerapos = new Vector3(transform.position.x, -1.0f,transform.position.z);
			Vector3 camerapos = new Vector3(transform.position.x, -1.0f,transform.position.z);
