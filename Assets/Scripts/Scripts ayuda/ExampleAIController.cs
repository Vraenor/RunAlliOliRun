...
public class ExampleAIController : MonoBehaviour {
	...
	private IAPowerUp IaPowerUpScript;
	...
	void Start () {
		...
		targetObject = waypoints [i]; // Set the first target as the first element of the waypoint list
		ObstaclesListRunTime = ObstaclesList.GetComponent<ObstaclesList> ().obstacles;
		IaPowerUpScript = this.GetComponent<IAPowerUp> ();
		boolTurbo = waypoints [0].GetComponent<WaypointNumber> ().turbo;
	...
	void OnTriggerEnter(Collider other)
	{
		...
		if (other.tag == "Waypoint") {
			...
			if (!activado) {
				...
				if (circuit == 1) 
				{
					boolTurbo = waypoints [i].GetComponent<WaypointNumber> ().turbo;
