...
public class EndRaceAndDisplayResults : MonoBehaviour {

	public List<Image> PortraitResults; //Images where later will be put the character's faces
	public KartController[] KartResults; //Direct reference of carOrder array: will be initialized later
	...
	void Start () {
		...
		for (int i = 0; i < KartResults.Length; i++) {
			if (KartResults [i].GetComponent<MyPowerUp> () != null)
				if (i >= 0 && i < 4) {
