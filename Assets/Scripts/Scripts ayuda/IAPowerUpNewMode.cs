...
public class IAPowerUpNewMode : MonoBehaviour {
	...
	private KartControllerNewMode[] allKarts; //The carts of this race
	...
	private List<int> powerUpsPairs = new List<int>();
	private List<int> powerUpsRacers = new List<int>();
	...
	void Start()
	{
		...
		powerUpsPairs.Add (3);
		powerUpsPairs.Sort ();
		powerUpsRacers.Add (0);
		powerUpsRacers.Add (3);
		powerUpsRacers.Add (4);
		powerUpsRacers.Sort ();
	...
	void OnTriggerEnter(Collider other)
	{
		...
		if (other.tag == "BoxPowerUp" && this.transform.GetChild(1).tag != "PowerUpDetector") {
			...
			if (this.gameObject.tag == "Racer") 
			{
				if (primero)
				{
					n_of_powerup = powerUpsRacers[0];
				}
				else
				{
					indexOfPowerUp = Random.Range(0,powerUpsRacers.Count);
					n_of_powerup = powerUpsRacers[indexOfPowerUp];
				}
			}

			else if (this.gameObject.tag == "Pair")
			{
				if (primero)
				{
					indexOfPowerUp = Random.Range(0,powerUpsPairs.Count - 1);
					n_of_powerup = powerUpsRacers[indexOfPowerUp];
				}
				else
				{
					indexOfPowerUp = Random.Range(0,powerUpsPairs.Count);
					n_of_powerup = powerUpsRacers[indexOfPowerUp];
				}
			}
//			if (primero)
...
//				n_of_powerup = Random.Range(0,powerups.Count + 1);
	...
	IEnumerator launchPowerUpAutomatically()
	{
		...
		case 0: //Normal turbo power up
			//TODO Comentar la linea de abajo si el turbo empieza a ir mal
			kart.SpeedBoost();
